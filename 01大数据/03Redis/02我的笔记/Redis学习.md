...
​		分布式：不同的多台服务器上面部署不同的服务模块（工程），他们之间通过Rpc/Rmi之间通信和调用，对外提供服务和组内协作。（一个任务需要多个节点共同完成，这个任务的执行方式就是分布式的）
...
各个版本的下载地址：http://download.redis.io/releases/。
...
如果需要进行大规模数据的恢复，且对于**数据恢复的完整性**不是非常敏感，那RDB方式要比AOF方式更加的高效。
...
​		所以，我们可以在一个队列中，一次性、顺序性、排他性的执行一系列命令。
