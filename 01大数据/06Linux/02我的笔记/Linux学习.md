...
tar （tape archive：档案打包）指令是打包指令，最后打包后的文件是 .tar.gz 的文件。（可以进行压缩和解压操作）。

基本语法：

```ini
tar  [选项]  XXX.tar.gz  打包的内容/目录   --打包目录，压缩后的文件格式.tar.gz。其中，-C 参数指定目标目录，默认解到当前目录下。
```

| 选项 |                    描述                     |
| :--: | :-----------------------------------------: |
|  -c  | （create 创建档案）**压缩**产生.tar打包文件 |
|  -v  |           （verbose）显示详细信息           |
|  -f  |         （file）指定压缩后的文件名          |
|  -z  |           （tar.gz）打包同时压缩            |
|  -x  |       （extract提取）**解压**.tar文件       |
