...
前端负责收集数据，组装好各种数据，但是真正的增删改查执行功能发送给后台，后台将前端收集来的数据转换成自己想要的数据后，再调用`service`来处理真正的功能。

使用最多的是`http`的`post`请求，该请求将数据转化为`json`，后端使用`@RequestBody`注解来取出请求体的参数，将其再转化为指定的对象。
